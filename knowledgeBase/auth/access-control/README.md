...
## [C-1] If multiple users call `DefaultSession::assertResults` all but the first caller lose their bonds
- Severity: `Critical`
...
## [C-2] Users can get their withdrawal active requests DoSed by malicious users
- Severity: `Critical`
...
## [C-3] `LivenessRecovery::setLivenessRecoveryOperator` will emit misleading event when role is not granted
- Severity: `Critical`
...
## [C-4] After the upgrade permissionless attacker can fully drain the L1 `TokenBridge` of `ERC20` tokens currently valued around $29M USD
- Severity: `Critical`
- Source report: `upgrade.md`
...
## [M-6] `AccountableAsyncRedeemVault` allows deposits for non-whitelisted or non-KYCed addresses
- Severity: `Medium`
...
## [M-7] InvestmentManager can use `AccountableFixedTerm::coverDefault` to misuse token approvals from anyone
- Severity: `Medium`
- Source report: `accountable.md`
...
## [M-9] Upgradeable contracts missing _disableInitializers() in constructors
- Severity: `Medium`
...
## [M-10] `AdminRegistry::acceptAdmin` leaves other roles on the outgoing admin
- Severity: `Medium`
...
## [M-11] `AdminRegistry::proposeAdmin` self-proposal permanently removes `DEFAULT_ADMIN_ROLE`
- Severity: `Medium`
- Source report: `clob.md`
...
## [M-12] `AdminRegistry` inherited `grantRole`/`revokeRole` bypass the two-step transfer guard
- Severity: `Medium`
- Source report: `clob.md`
...
## [M-13] Misleading revert message in onlyUser modifier
- Severity: `Medium`
...
## [M-14] Premature zeroing of epoch rewards in `claimUndistributedRewards` can block legitimate claims
- Severity: `Medium`
...
## [M-15] Use unchecked block for increment operations in `distributeRewards`
- Severity: `Medium`
- Source report: `core.md`
...
## [M-20] Redundant `FUNDER_ROLE` in `LineaRollupYieldExtension`
- Severity: `Medium`
...
## [M-22] `SettersGovernor::setWhitelistStatus` allows values other than 0 and 1 potentially leading to DOS
- Severity: `Medium`
...
## [M-23] Check return value when calling `Allowlist::exchangeAllowed` and `RemoraToken::_exchangeAllowed` to prevent unauthorized transfers
- Severity: `Medium`
...
## [M-24] `AccountableYield::setNavGracePeriod` uses `Unauthorized` error for invalid input
- Severity: `Medium`
...
## [M-26] Missing modifiers on `YieldStrategyFactory.createYieldStrategy` can lead to deployment of unverified strategies
- Severity: `Medium`
- Source report: `pr50.md`
...
## [M-28] `SecuritizeOnRamp::swap` and `SecuritizeOffRamp::redeem` pass operator as investor address resulting in denial of service
- Severity: `Medium`
...
## [M-29] Incorrect use of `investorExists` modifier in `PublicStockOnRamp::swap`
- Severity: `Medium`
- Source report: `ramp.md`
...
## [M-30] Missing `liquidityToken` validation in `CollateralLiquidityProvider::initialize`
- Severity: `Medium`
- Source report: `ramp.md`
...
## [M-31] Redundant Access Control Check in `SecuritizeInternalNavProvider::addRateUpdater, removeRateUpdater`
- Severity: `Medium`
- Source report: `ramp.md`
...
## [M-32] `TrustService::removeRole` doesn't delete already owned entities so address which lost role can still manage existing entities
- Severity: `Medium`
...
## [M-34] Investor can prevent themselves from being removed by making `removeInvestor` revert
- Severity: `Medium`
- Source report: `rebasing.md`
...
## [M-35] Resolve inconsistency between `DSToken::checkWalletsForList` and `RegistryService::removeWallet`
- Severity: `Medium`
- Source report: `rebasing.md`
...
## [M-38] Incomplete mapping updates in `setVault` function cause vault address inconsistencies
- Severity: `Medium`
...
## [M-41] `STBL_Register::addAsset` does not check for non-empty asset name
- Severity: `Medium`
...
## [M-42] Missing check if `receiver` is whitelisted in `StakingVault::mint, deposit`
- Severity: `Medium`
...
## [M-45] Combination of Ownable and AccessControl can cause loss of admin functionality
- Severity: `Medium`
...
## [M-48] `PocketFactory::approveTailor` doesn't verify `ITailor` interface implementation
- Severity: `Medium`
