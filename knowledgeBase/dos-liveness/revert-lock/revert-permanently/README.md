...
## 5. Don't Add Duplicate `documentHash` to `DocumentManager::DocumentStorage::_docHashes` When Overwriting via `_setDocument` as This Causes Panic Revert When Calling `_removeDocument`
...
## 6. Excess Contributions Become Permanently Locked Due to Non-Exact Deposit Enforcement
...
## 7. Excessive `maximumContestants` Could Make Games Revert in `DefaultSession::recordResults` Due to Out of Gas
...
## 8. `Getters::getCollateralSurplus` Returns Positive Values Even When `Surplus::processSurplus` Is Guaranteed to Revert
...
## 9. `IERC7160` Specification Requires `hasPinnedTokenURI` to Revert for Non-Existent `tokenId`
...
## 10. `IERC7160` Specification Requires `pinTokenURI` to Revert for Non-Existent `tokenId`
...
## 11. Insufficient Validation in `AvalancheL1Middleware::removeOperator` Can Create Permanent Validator Lockup
...
## 12. No Way to Revert `setInvestorLiquidateOnly`
...
## 13. Revert Fast by Performing Input-Related Checks Prior to Storage Reads and External Calls
...
## 14. `RewardHandler` May Revert Due to Receiving Less Than Expected
...
## 15. Same Wallet Can Be Added Multiple Times to an Investor, Artificially Increasing Their Wallet Count Causing Adding New Wallets to Revert
...
## 16. Unbounded `depositAddresses` Can Cause `CompliantDepositRegistry::challengeLatestBatch` to Revert Due to Out of Gas
...
## 17. Users Can Reset the Status of Their `firstPurchase` on the `referralData` When the Stablecoin Doesn't Revert on Transfers to `address(0)`
