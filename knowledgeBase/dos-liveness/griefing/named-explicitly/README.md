...
## 4. Changing Investor Country to the Same Country Inflates Investor Count Erroneously Triggering Max Investor Errors

**Severity:** 🟡 Medium
**Source:** `cyfrin/rebasing.md`

**Description:**
When `RegistryService::updateInvestor` is called — even if only non-country fields change — it invokes `ComplianceServiceRegulated::adjustInvestorCountsAfterCountryChange`. This function increments the count for the new country without checking whether the new country is the same as the old country. If an admin calls `updateInvestor` with the same country value (e.g., updating only the investor name), the compliance service increments the US investor count as if the investor moved into that category from another jurisdiction. Repeated such operations inflate the count beyond the actual number of investors in that category, eventually causing the `MAX_INVESTORS_IN_CATEGORY` check to trigger falsely and blocking legitimate token transfers.

**Impact:**
Routine admin updates that change non-country investor attributes can silently inflate jurisdiction investor counts. Once counts are inflated, `completeTransferCheck` will erroneously reject transfers even when the investor count is actually within limits, causing a DoS on token transfers for that category.

**Recommended Mitigation:**
In `adjustInvestorCountsAfterCountryChange`, add a guard to return early without modifying counts when `newCountry == oldCountry`.
...
**描述：**
當調用 `RegistryService::updateInvestor` 時——即使只有非國家字段發生變化——它也會調用 `ComplianceServiceRegulated::adjustInvestorCountsAfterCountryChange`。此函數遞增新國家的計數，而不檢查新國家是否與舊國家相同。如果管理員用相同的國家值調用 `updateInvestor`（例如，只更新投資者姓名），合規服務會遞增美國投資者計數，彷彿投資者從另一個管轄區進入該類別。重複此類操作會將計數膨脹超過該類別的實際投資者數量，最終導致 `MAX_INVESTORS_IN_CATEGORY` 檢查錯誤觸發並阻止合法的代幣轉移。

**影響：**
更改非國家投資者屬性的例行管理員更新可能靜默地膨脹管轄區投資者計數。一旦計數被膨脹，`completeTransferCheck` 將錯誤地拒絕轉移，即使投資者計數實際上在限制範圍內，為該類別的代幣轉移造成 DoS。

**修復建議：**
在 `adjustInvestorCountsAfterCountryChange` 中，添加一個守衛，當 `newCountry == oldCountry` 時提前返回而不修改計數。

---

## 5. Cooldown Contracts Underreport the Real Balance of Users Because They Only Consider the Balance of Requests Whose Cooldown Period Is Over
...
## 6. Deployment Script Requires Unencrypted Private Keys

**Severity:** 🟡 Medium
**Source:** `cyfrin/trade.md`

**Description:**
Multiple deployment and operational scripts in the `BasisTrade` codebase — `DeployBasisTradeTailor.s.sol`, `DeployBasisTradeVault.s.sol`, `DeployMockPocketOracle.s.sol`, and `DeployMocks.s.sol` — all read the deployer private key (and in some cases an admin key) from plain-text environment variables via `vm.envUint("DEPLOYER_PRIVATE_KEY")` and pass it directly to `vm.startBroadcast(deployerPrivateKey)`. Storing raw private keys in `.env` files creates an operational security risk through multiple leakage vectors: version control history, shell history, CI/CD logs, backup systems, and compromised developer machines.

**Impact:**
Exposure of the deployer or admin private key allows an attacker to deploy malicious contracts, modify access control configurations, or drain protocol funds. The risk is elevated when multiple scripts across a deployment pipeline share the same key path.

**Recommended Mitigation:**
Import all deployer keys into Foundry's encrypted keystore using `cast wallet import`. Reference the keystore account in scripts using `--account` and call `vm.startBroadcast()` without any private key argument, so keys are never stored in plaintext.
...
**描述：**
`BasisTrade` 代碼庫中的多個部署和操作腳本——`DeployBasisTradeTailor.s.sol`、`DeployBasisTradeVault.s.sol`、`DeployMockPocketOracle.s.sol` 和 `DeployMocks.s.sol`——都通過 `vm.envUint("DEPLOYER_PRIVATE_KEY")` 從明文環境變量讀取部署者私鑰（在某些情況下還有管理員密鑰），並直接傳遞給 `vm.startBroadcast(deployerPrivateKey)`。將原始私鑰存儲在 `.env` 文件中通過多種洩漏媒介造成操作安全風險：版本控制歷史、Shell 歷史、CI/CD 日誌、備份系統和受感染的開發者機器。

**影響：**
暴露部署者或管理員私鑰允許攻擊者部署惡意合約、修改訪問控制配置或耗盡協議資金。當部署管道中的多個腳本共享相同的密鑰路徑時，風險更高。

**修復建議：**
使用 `cast wallet import` 將所有部署者密鑰導入 Foundry 的加密密鑰庫。在腳本中使用 `--account` 引用密鑰庫賬戶，並調用不帶任何私鑰參數的 `vm.startBroadcast()`，以避免密鑰以明文存儲。

---

## 7. Dynamic LP Fees Will Remain Zero by Default Unless Explicitly Updated
...
## 8. External LST Liability Settlements Are Lost to the Protocol When Ossification and Yield Provider Removal Precedes Yield Reporting
...
## 9. Mismatch Between Tier Assignment and Enumeration Breaks Tier Determinism
...
## 10. `MyriadCTFExchange.filledAmounts` Mapping Slot and `hashOrder` Computed Multiple Times Per Order
...
## 11. Refactor Away Duplicated Code Between `ComplianceService::newPreTransferCheck` and `preTransferCheck`
...
## 12. Resolve Inconsistency Between `DSToken::checkWalletsForList` and `RegistryService::removeWallet`
...
## 13. `TransactionRelayer` and `SecuritizeSwap` Should Use `CommonUtils::encodeString`
...
## 14. Use `uint128` to Pack `DepositManager::protocolFee`, `maxCreatorFee` Into the Same Storage Slot
...
## 15. Use Named Constants to Indicate Purpose of Magic Numbers
...
## 17. Use Named Imports (syntetika)
...
## 18. Use Named Mapping Parameters to Explicitly Note the Purpose of Keys and Values (rebasing)
...
## 19. Use Named Mapping Parameters to Explicitly Note the Purpose of Keys and Values (syntetika)
...
## 20. Use Named Mapping Parameters to Explicitly Note the Purpose of Keys and Values (trade)
...
## 21. Use Named Mappings to Explicitly Denote the Purpose of Keys and Values (predeposit)
...
## 22. Use Named Mappings to Explicitly Denote the Purpose of Keys and Values (protocol)
...
## 23. Use Named Mappings to Explicitly Indicate the Purpose of Keys and Values (wannabetv2)
...
## 24. `YieldManager::fundYieldProvider` and `LidoStVaultYieldProvider::fundYieldProvider` Don't Enforce `isStakingPaused` and `isOssificationInitiated` Allowing Unsafe Staking

**Severity:** 🟡 Medium
**Source:** `cyfrin/manager.md`

**Description:**
The protocol specification states that new validator deposits must be paused when ossification is initiated, LST liabilities are incurred, or withdrawal deficits exist. The `isStakingPaused` flag is set by explicit admin calls and is also automatically set via `_pauseStakingIfNotAlready` during `withdrawLST` and `initiateOssification`. However, neither `YieldManager::fundYieldProvider` nor `LidoStVaultYieldProvider::fundYieldProvider` checks `isStakingPaused`. `LidoStVaultYieldProvider::fundYieldProvider` does revert if `isOssified` is fully set, but neither function checks the earlier `isOssificationInitiated` flag which fires when ossification begins but before completion. This allows fresh staking deposits to flow into yield providers during periods when the protocol considers staking unsafe.

**Impact:**
New funds can be staked even when staking has been explicitly paused, when liabilities are active, or when ossification has been initiated. This violates the protocol's safety invariant that staking stops under those conditions, potentially locking additional user funds into a yield provider that is being wound down.

**Recommended Mitigation:**
Add checks for both `isStakingPaused` and `isOssificationInitiated` in `LidoStVaultYieldProvider::fundYieldProvider`, alongside the existing `isOssified` check.
...
**描述：**
協議規範規定，當啟動骨化、發生 LST 負債或存在提款赤字時，新的驗證者存款必須暫停。`isStakingPaused` 標誌由明確的管理員調用設置，也在 `withdrawLST` 和 `initiateOssification` 期間通過 `_pauseStakingIfNotAlready` 自動設置。然而，`YieldManager::fundYieldProvider` 和 `LidoStVaultYieldProvider::fundYieldProvider` 都不檢查 `isStakingPaused`。`LidoStVaultYieldProvider::fundYieldProvider` 確實在 `isOssified` 完全設置時回退，但兩個函數都不檢查 `isOssificationInitiated` 標誌，該標誌在骨化開始但完成之前觸發。

**影響：**
即使在明確暫停質押、負債活躍或已啟動骨化時，也可以存入新資金。這違反了協議在這些條件下停止質押的安全不變量，可能將額外的用戶資金鎖定到正在清算的收益提供者中。

**修復建議：**
在 `LidoStVaultYieldProvider::fundYieldProvider` 中添加對 `isStakingPaused` 和 `isOssificationInitiated` 的檢查，與現有的 `isOssified` 檢查一起。

---

## 25. `eciesjs` Major Version Mismatch Between dApp SDK and Mobile Wallet Creates Untested Cryptographic Interoperability Risk
...
## 26. `yUSDeVault` Edge Cases Should Be Explicitly Handled to Prevent View Functions From Reverting
