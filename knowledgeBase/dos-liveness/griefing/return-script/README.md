...
## 9. `CompensationPriceFinder::getZeroForOne` May Compute Smaller Effective Prices Than Expected
...
## 10. Consider Using `SafeCast` When Downcasting Amounts
...
## 11. Consider Using Exponential Notation in Tests
...
## 12. Consider Using Named Mapping Parameters
...
## 13. Deploy Script `UpdateParallelizer.ts` Does Not Handle Facet Removal Case
...
## 14. Deployment Script Requires Unencrypted Private Key
...
## 15. `DocumentManager::hasSignedDocs` Incorrectly Returns `true` When There Are No Documents to Sign
...
## 16. `IBridgeableTokenP::swapLzTokenToPrincipalToken` Interface Declares a `uint256` Return Value but Implementation Returns Nothing
...
## 17. Only Update `deployedAssets` When `remaining > 0` in `AccountableYield::repay`
...
## 18. Order Not Eligible at `eligibleAt`
...
## 19. `PledgeManager::refundTokens` Doesn't Decrement `tokensSold` When Pledge Hasn't Concluded
...
## 21. Prefer Explicit Unsigned Integer Sizes
...
## 22. Prefer Named Return Parameters, Especially for `memory` Returns
...
## 23. Refactor Duplicated Checks Into Modifiers
...
## 24. Remove Obsolete `onlyTokenOwner` From `_transferToNftReceiver`
...
## 25. Remove Return Value From `DSToken::updateInvestorBalance` as It Is Never Checked
...
## 26. Return Fast in `ComplianceServiceRegulated::checkHoldUp` If Platform Wallet
...
## 27. `SDLVesting::withdrawRESDLPositions` Enhancements
...
## 28. Scaling `winningThreshold` Incorrectly Reduces Randomness Distribution
...
## 32. Unresolved Developer Comments
...
## 33. Use `type(uint256).max` When Withdrawing From Aave
