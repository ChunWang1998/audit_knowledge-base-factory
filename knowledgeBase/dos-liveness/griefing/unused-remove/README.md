...
**Description:**
`LineaRollup.sol` contains a leftover developer comment `// TODO - Add access control to proxy admin only` on line 38. Such TODO comments indicate unresolved design decisions or unimplemented features that were intended for completion before production deployment. Their presence in a production codebase suggests incomplete implementation and can mislead auditors about whether the described functionality has been addressed.

**Impact:**
The comment suggests a proxy admin access control restriction that may not have been implemented, potentially representing a missing security control. At minimum it adds noise and suggests incomplete state.

**Recommended Mitigation:**
Resolve the TODO by either implementing the described access control or confirming it is intentionally not needed and removing the comment accordingly.
...
**描述：**
`LineaRollup.sol` 第 38 行包含一條殘留開發者注釋 `// TODO - Add access control to proxy admin only`。此類 TODO 注釋表明存在未解決的設計決策或預計在生產部署前完成的未實現功能。其在生產代碼庫中的存在表明實現不完整，並可能誤導審計者認為所描述的功能已被處理。

**影響：**
注釋表明可能未實現代理管理員訪問控制限制，潛在代表缺失的安全控制。即使不存在安全問題，也會增加噪音並表明代碼處於不完整狀態。

**修復建議：**
通過實現所描述的訪問控制或確認故意不需要並相應移除注釋來解決該 TODO。

---

## 4. Remove unused constant `CryptoartNFT::ROYALTY_BASE`
...
## 5. Remove useless function `ComplianceServiceRegulated::adjustTransferCounts`
...
## 7. Unused custom error should be removed if not required
...
## 8. Unused error `IBet::InvalidAmount`
...
## 11. Unused library and struct definitions increase deployment costs and reduce code clarity
